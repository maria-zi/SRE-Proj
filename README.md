...
Nginx (Reverse Proxy)	80	Входная точка, балансировка	- Проксирует запросы → FastAPI
- Распределяет нагрузку между 3 репликами
- Static files, SSL termination
FastAPI App	8000	REST API (3 реплики, HPA 3-10)	- /clients — CRUD клиентов
- /reports/top-clients — TOP-10 (CTE + Window)
- /reports/daily-revenue — выручка за 30 дней
- /metrics — Prometheus metrics
- Redis кэш (5 минут)
PostgreSQL	5432	Основная база данных	- clients — клиенты
- services — услуги
- payments — платежи
- Индексы, CTE, оконные функции
Redis	6379	Кэш ответов API	- Кэширует ответы на 5 минут
- Ускоряет GET /clients (5 мин кэш)
Prometheus	9090	Сбор метрик	- Метрики API (requests, latency)
- CPU, память container
- PostgreSQL metrics
- Node/cAdvisor metrics
Grafana	3000	Визуализация и алерты	- Дашборды (CPU, память, запросы)
- Алерты (CPU > 80%, Memory > 90%)
- Импортированные дашборды (1860, 193, 9628)
