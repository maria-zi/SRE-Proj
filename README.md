...
└────────────────────────────┬────────────────────────────────────┘
                             ...
                             ▼
┌─────────────────────────────────────────────────────────────────┐
│              FastAPI App (3 replicas, HPA 3-10)                 │
│              • /clients — CRUD клиентов                         │
│              • /reports/top-clients — TOP-10 (CTE + Window)     │
│              • /reports/daily-revenue — выручка за 30 дней      │
...
Пользователь делает запрос → Nginx

Nginx проксирует → FastAPI (балансировка между 3 репликами)

FastAPI проверяет Redis кэш → если нет → PostgreSQL

PostgreSQL выполняет запрос (с индексами, CTE, оконными функциями)

Prometheus собирает метрики (CPU, память, запросы/сек, latency)

Grafana визуализирует (дашборды, алерты)
...
Если нет — устанавливает Docker + docker-compose

Запускает 4 контейнера: API, PostgreSQL, Redis, Nginx
...
Устанавливает Helm (если нет)

Добавляет репозиторий Bitnami

Создаёт namespace telecom

Запускает Helm chart (устанавливает API + PostgreSQL + Redis)

Ждёт готовности подов
...
Введи ID: 1860 (Node Exporter) → Load → Save

Теперь в меню дашборд Node Exporter — покажет CPU, память, диск, сеть
