...
1. **Пользователь** делает запрос → Nginx
2. **Nginx** проксирует → FastAPI (балансировка между 3 репликами)
3. **FastAPI** проверяет **Redis кэш** → если нет → PostgreSQL
4. **PostgreSQL** выполняет запрос (с индексами, CTE, оконными функциями)
5. **Prometheus** собирает метрики (CPU, память, запросы/сек, latency)
6. **Grafana** визуализирует (дашборды, алерты)
